...
Exécuter sur une install avec le **plugin** actif, permaliens rechargés (*Réglages → Permaliens* → Enregistrer), et de préférence `wp wpis seed_demo` (ou données réelles). Cocher en local.

| URL / contexte | Attendu |
| -------------- | ------- |
| `/` (page d’accueil) | Le modèle *Front Page* s’applique ; le contenu vient de la page d’accueil (souvent *home* en Query sur `quote`). `feed-demo.js` **n’est plus** enregistré ici. |

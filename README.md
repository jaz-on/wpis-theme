...
- WordPress **6.9+**
- PHP **8.2+**
- The **WordPress Is… Core** package (`wpis-core`, main file `wpis-core.php`) when you wire dynamic quotes, submissions and REST. The theme runs standalone for static screen content until **wpis-core** is active.
...
3. **Manifest pages are not created on activation** (same idea as Twenty Twenty). **In the admin:** go to **Appearance → WPIS import** (`themes.php?page=wpis-import`) and use **Import pages**, **Remove pages**, **Run reset**, and (with **wpis-core** active) **Sample quotes** actions. With **WP-CLI**: **`wp wpis-seed import`** (overwrites existing page bodies from files by default; use `--no-sync` to skip), **`wp wpis-seed clean`** (`--force` to delete permanently), or **`wp wpis-seed reset`** (clean then import), plus **`wp wpis seed_quotes`** for sample quotes (`seed_starter` and `seed_demo` stay as aliases). Seeded pages follow `content/html/` (including **privacy** and **profile**). **My profile** uses the **`[wpis_profile]`** shortcode from **wpis-core**; **submit** uses **`[wpis_submit_form]`**. This sets **Reading** to static **Home** unless you uncheck the box in the admin or pass **`--no-reading`** in the CLI. **Navigation** is edited in **Appearance → Editor** (Header template part); there is no classic menu location.
...
**WPIS import (CLI):** `wp wpis-seed help` (requires [WP-CLI](https://wp-cli.org/) with this theme active).
